...
class ConsultantController(metaclass=SingletonMeta):
  ...
  def prepare_system(self) -> None:
    ...
    self.embedding = EmbeddingModelsManager(self.name_embedding_model, False).get_huggingface_embedding_model()
    self.database_manager = VectorDatabaseManager(self.embedding)
  ...
  def is_system_configured(self) -> bool:
    ...
    embedding_conditions = (self.name_embedding_model is not None) and (self.embedding is not None)
    #database_conditions = (self.database_manager is not None) and (len(self.database_manager.get_list_collections()) > 0)
    required_parameters = [self.init, llm_conditions_1, llm_conditions_2, embedding_conditions]
  ...
  def answer_query(self, query:str) -> Dict | None:
    if self.is_system_configured():
      self.prepare_system()
      keywords = self.get_keywords(query=query, iterations=1, max_execution_time=10000)
      possible_error_messag = "Agent stopped due to iteration limit or time limit."
      if ((keywords == possible_error_messag) and (keywords_for_RAG is not None)):
        keywords = keywords_for_RAG
      context_docs = self.query_database(query=keywords, return_retriever=False, optimized_database_query=False)
      prompt = self.create_prompt(system_prompt_query_responder)
      chain = create_stuff_documents_chain(self.llm, prompt)
      answer = chain.invoke({'input' : query, "context": context_docs})
      return {"output": answer ,"context_docs": context_docs, "keywords": keywords}
    else:
      return None
  ...
  def get_compression_retriever(self, retriever:VectorStoreRetriever) -> ContextualCompressionRetriever:
    compressor = FlashrankRerank(top_n=self.k_index)
